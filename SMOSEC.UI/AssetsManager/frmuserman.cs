...
{
    partial class frmuserman : Smobiler.Core.Controls.MobileForm
    {
        ...
        public void Bind()
        {
            ...
            {
                //获取所有部门数据
                List<cmdb_useman> listUser = AutofacConfig.assUserService.GetAll();
                foreach (cmdb_useman user in listUser)
                {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("该使用人已经在列表中!");
                }
                string dealman = Client.Session["local_username"].ToString();
                ReturnInfo returnInfo = AutofacConfig.assUserService.Adduserman(txtcreate.Text, dealman);
