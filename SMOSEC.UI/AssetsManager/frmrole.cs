...
{
    partial class frmrole : Smobiler.Core.Controls.MobileForm
    {
        public frmrole() : base()
        {
        ...
        public void Bind()
        {
            ...
            {
                //获取所有部门数据
                List<cmdb_role> listUser = AutofacConfig.assRoleService.GetAll();
                foreach (cmdb_role user in listUser)
                {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("该角色已经在列表中!");
                }
                string dealman = Client.Session["local_username"].ToString();
                ReturnInfo returnInfo = AutofacConfig.assRoleService.Addrole(txtcreate.Text, dealman);
