...
{
    partial class frmpro : Smobiler.Core.Controls.MobileForm
    {
        public frmpro() : base()
        {
        ...
        public void Bind()
        {
            ...
            {
                //获取所有部门数据
                List<cmdb_project> listUser = AutofacConfig.assProService.GetAll();
                foreach (cmdb_project user in listUser)
                {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("该项目已经在列表中!");
                }
                string dealman = Client.Session["local_username"].ToString();
                ReturnInfo returnInfo = AutofacConfig.assProService.Addpro(txtcreate.Text, dealman);
