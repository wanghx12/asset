...
{
    partial class frmteam : Smobiler.Core.Controls.MobileForm
    {
        public frmteam() : base()
        {
        ...
        public void Bind()
        {
            ...
            {
                //获取所有部门数据
                List<cmdb_team> listUser = AutofacConfig.assTeamService.GetAll();
                foreach (cmdb_team user in listUser)
                {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("该团队已经在列表中!");
                }
                string dealman = Client.Session["local_username"].ToString();
                ReturnInfo returnInfo = AutofacConfig.assTeamService.Addteam(txtcreate.Text, dealman);
