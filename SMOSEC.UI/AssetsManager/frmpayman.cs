...
namespace SMOSEC.UI.AssetsManager
{
    partial class frmpayman : Smobiler.Core.Controls.MobileForm
    {
        ...
        public void Bind()
        {
            ...
            {
                //获取所有部门数据
                List<cmdb_payman> listUser = AutofacConfig.assPaymanService.GetAll();
                foreach (cmdb_payman user in listUser)
                {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("该挂账人已经在列表中!");
                }
                string dealman = Client.Session["local_username"].ToString();
                ReturnInfo returnInfo = AutofacConfig.assPaymanService.Addpayman(txtcreate.Text, dealman);
