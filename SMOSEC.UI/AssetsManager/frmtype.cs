...
{
    partial class frmtype : Smobiler.Core.Controls.MobileForm
    {
        public frmtype() : base()
        {
        ...
        public void Bind()
        {
            ...
            {
                //获取所有部门数据
                List<cmdb_assettype> listUser = AutofacConfig.assTypeService.GetAll();
                foreach (cmdb_assettype user in listUser)
                {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("该类型已经在列表中!");
                }
                string dealman = Client.Session["local_username"].ToString();
                ReturnInfo returnInfo = AutofacConfig.assTypeService.Addtype(txtcreate.Text, dealman);
