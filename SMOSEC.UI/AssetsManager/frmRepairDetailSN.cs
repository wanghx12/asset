...
namespace SMOSEC.UI.AssetsManager
{
    partial class frmRepairDetailSN : Smobiler.Core.Controls.MobileForm
    {
        ...
        private void btnSave_Press(object sender, EventArgs e)
        {
            try
            {
                ReturnInfo r = autofacConfig.assRepairOrderService.UpdateAssRepairOrder(ROID, "维修完毕");
                if (r.IsSuccess)
                {
        ...
        private void btnSave1_Press(object sender, EventArgs e)
        {
            try
            {
                ReturnInfo r = autofacConfig.assRepairOrderService.UpdateAssRepairOrder(ROID, "维修失败");
                if (r.IsSuccess)
                {
                    ShowResult = ShowResult.Yes;
                    ClearInfo();
