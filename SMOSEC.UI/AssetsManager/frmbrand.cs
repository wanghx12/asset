...
{
    partial class frmbrand : Smobiler.Core.Controls.MobileForm
    {
        ...
        public void Bind()
        {
            ...
            {
                //获取所有部门数据
                List<cmdb_brand> listUser = AutofacConfig.assBrandService.GetAll();
                foreach (cmdb_brand user in listUser)
                {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    throw new Exception("该型号已经在列表中!");
                }
                string dealman = Client.Session["local_username"].ToString();
                ReturnInfo returnInfo = AutofacConfig.assBrandService.Addbrand(txtcreate.Text, dealman);
